...
class GetRekapPenanganan @Inject constructor(private val rekapitulasiRepository: RekapitulasiRepository) :
    UseCase<List<Lubang>, None>() {
    override suspend fun run(params: None): Either<Failure, List<Lubang>> {
        return rekapitulasiRepository.getRekapitulasiPenanganan()
    }
}