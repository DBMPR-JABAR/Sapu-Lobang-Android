...
import id.go.jabarprov.dbmpr.surveisapulubang.databinding.FragmentListPotensiBinding
...
class DetailSurveiPotensiFragment : Fragment() {
    ...
    private lateinit var binding: FragmentListPotensiBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {
        binding = FragmentListPotensiBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun processResultSurveiState(state: Resource<ResultSurvei>) {
        when (state) {
            ...
            is Resource.Success -> {
                if (state.data.listPotensiLubang.isEmpty()) {
                    binding.apply {
                        recyclerViewListPotensi.isVisible = false
                ...
                resultSurveiListLubangAdapter.submitList(state.data.listPotensiLubang)
            }
            else -> Unit
        }
    }
